...
function renderHeaderNameAndSkills(){
  ...
  const nameAndSkillHTML = `
    <p class="name">${name}</p>        
    <div class="scroller">
      <div class="main-skills scroller__inner">${mainSkills}</div>
    </div>
    `;
...
function renderHomeSectionContents(){
  ...
  let homeSectionHTML = `
     <div class="profilePic-and-skills-container">
        ...
        <p class="bio">${bioWithoutLastThreeWords} <span class="unique-font">${bioHighlightedWords}<br>${bioFinalWord}.</span></p>

        <a href="data/${resume}"  download="${resume}" class="download-resume-btn glowing-button">
...
export {renderMainPageContents, renderProjectFullView, renderHeaderNameAndSkills, renderFooterContents}